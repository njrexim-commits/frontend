    ...
    const renderEditorFields = (obj: Record<string, unknown>, prefix = "") => {
        return Object.keys(obj).map((key) => {
            ...
            if (typeof value === "object" && value !== null && !Array.isArray(value)) {
                return (
                    <div key={fullPath} className="space-y-4 border-l-2 border-slate-100 pl-4 py-2">
                        <h4 className="text-sm font-bold uppercase tracking-wider text-primary">{key}</h4>
            ...
            if (Array.isArray(value)) {
                return (
                    <div key={fullPath} className="space-y-4 border-l-2 border-primary/20 pl-4 py-2 bg-slate-50/50 rounded-r-lg">
                        <h4 className="text-sm font-bold uppercase tracking-wider text-primary">{key} (Array)</h4>
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                    <h1 className="text-3xl font-extrabold tracking-tight text-secondary">Page Content Editor</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredPages.map((page) => (
                    <Card key={page._id} className="group hover:shadow-xl transition-all duration-300 border-none shadow-sm overflow-hidden bg-white">
                        <CardHeader className="bg-secondary text-white pb-6 relative overflow-hidden">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-hidden p-0 border-none shadow-2xl">
                    <div className="bg-secondary px-8 py-6 flex items-center justify-between shrink-0">
                        <div className="space-y-1">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden bg-white">
                        ...
                        <div className="shrink-0 px-8 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-end gap-3">
                            ...
                            <Button type="submit" className="bg-primary hover:bg-primary/90 px-8 gap-2">
